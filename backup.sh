...
restic backup /data ${RESTIC_JOB_ARGS} -o rclone.args="serve restic --stdio --b2-hard-delete --drive-use-trash=false --fast-list --transfers=32" --tag=${RESTIC_TAG?"Missing environment variable RESTIC_TAG"} >> ${lastLogfile} 2>&1
backupRC=$?
...
if [[ $backupRC == 0 ]] && [ -n "${RESTIC_FORGET_ARGS}" ]; then
    ...
    restic forget ${RESTIC_FORGET_ARGS} -o rclone.args="serve restic --stdio --b2-hard-delete --drive-use-trash=false --fast-list --transfers=32" >> ${lastLogfile} 2>&1
    rc=$?
