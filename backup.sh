...
restic backup /data ${RESTIC_JOB_ARGS} -o rclone.args="serve restic --stdio --b2-hard-delete --drive-use-trash=false --fast-list --b2-endpoint=s3.eu-central-003.backblazeb2.com" --tag=${RESTIC_TAG?"Missing environment variable RESTIC_TAG"} >> ${lastLogfile} 2>&1
backupRC=$?
...
if [[ $backupRC == 0 ]] && [ -n "${RESTIC_FORGET_ARGS}" ]; then
    ...
    restic forget ${RESTIC_FORGET_ARGS} -o rclone.args="serve restic --stdio --b2-hard-delete --drive-use-trash=false --fast-list --b2-endpoint=s3.eu-central-003.backblazeb2.com" >> ${lastLogfile} 2>&1
    rc=$?
